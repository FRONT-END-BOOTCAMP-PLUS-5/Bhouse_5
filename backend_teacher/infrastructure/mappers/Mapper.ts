import { MemberTable, CategoryTable, MenuTable, MenuImageTable, RoleTable, MemberRoleTable } from '../types/database'
...
export class Mapper {
  ...
  static toCategory(source: CategoryTable): Category {
    return new Category(source.id, source.name, source.is_public, new Date(source.created_at), source.order)
  }
  ...
  static toMenuImage(source: MenuImageTable): MenuImage {
    return new MenuImage(source.id, source.name, source.is_default, source.menu_id)
  }
  ...
  static toMemberRole(source: MemberRoleTable): MemberRole {
    return new MemberRole(source.member_id, source.role_id, new Date(source.created_at))
  }
}
