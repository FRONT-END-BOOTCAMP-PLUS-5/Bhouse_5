...
export class SbMemberRoleRepository implements MemberRoleRepository {
  ...
  async findAll(): Promise<MemberRole[]> {
    ...
    // === 3. 데이터 변환해서 담기
    const memberRoles: MemberRole[] = result.map((item) => Mapper.toMemberRole(item))
    return memberRoles
  }
  ...
  async count(): Promise<number> {
    // === 1. 쿼리 빌드하기
    const query = this.supabase.from('member_roles').select('*', { count: 'exact' })
  ...
  async delete(memberId: string, roleId: number): Promise<void> {
    const { error } = await this.supabase.from('member_roles').delete().eq('member_id', memberId).eq('role_id', roleId)
    if (error) throw new Error(error.message)
  }
}
