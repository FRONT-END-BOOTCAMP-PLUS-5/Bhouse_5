...
export class SbMenuRepository implements MenuRepository {
  ...
  private buildMenuQuery(
    ...
    let query = this.supabase.from(base).select(selectStr, options?.isCount ? { count: 'exact' } : undefined)
    ...
    if (criteria.searchWord) {
      query = query.or(`kor_name.ilike.%${criteria.searchWord}%,eng_name.ilike.%${criteria.searchWord}%`)
    }
  ...
  async findAll(criteria: MenuSearchCriteria, relations?: MenuRelationsOptions): Promise<Menu[]> {
    // === 1. 쿼리 빌드하기
    let query = null
    {
    ...
    if (relations?.includeImages && result && Array.isArray(result)) {
      ...
      menus.forEach((menu, idx) => {
        ...
        if (raw.menu_images && Array.isArray(raw.menu_images)) {
          ...
          menu.menuImages = raw.menu_images.map((img) => Mapper.toMenuImage(img)) as MenuImage[]
        }
      })
    }
  ...
  async findById(id: number, relations?: MenuRelationsOptions): Promise<Menu | null> {
    ...
    let query = null
    {
      ...
      // 쿼리 빌더 사용
      query = this.supabase.from('menus').select(selectStr).eq('id', id).single()
    }
  ...
  async update(menu: Menu): Promise<Menu> {
    ...
    const { data, error } = await this.supabase.from('menus').update(menu).eq('id', menu.id).select().single()
    if (error) throw new Error(error.message)
