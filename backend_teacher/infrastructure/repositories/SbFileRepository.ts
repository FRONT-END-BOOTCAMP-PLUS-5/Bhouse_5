...
import { FileRepository, FileMetadata, FileSaveOptions } from '../../domain/repositories/FileRepository'
...
export class SbFileRepository implements FileRepository {
  ...
  async save(
    ...
    const fullPath = path.endsWith('/') ? path + finalFileName : path + '/' + finalFileName
    const { error } = await this.supabase.storage.from(this.bucket).upload(fullPath, file, {
      cacheControl: '3600',
    ...
    const { data: urlData } = this.supabase.storage.from(this.bucket).getPublicUrl(fullPath)
    return urlData.publicUrl
  }

  async delete(path: string, fileName: string): Promise<void> {
    const fullPath = path.endsWith('/') ? path + fileName : path + '/' + fileName
    const { error } = await this.supabase.storage.from(this.bucket).remove([fullPath])
    if (error) {
